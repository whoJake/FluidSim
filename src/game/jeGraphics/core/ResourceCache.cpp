...
{
    return request_resource(m_state.shaderModules, m_device, stage, glslSource, entryPoint);
}

Framebuffer& ResourceCache::request_framebuffer(const RenderTarget& renderTarget, const RenderPass& renderPass)
{
    return request_resource(m_state.framebuffers, m_device, renderTarget, renderPass);
}
...
{
    return request_resource(m_state.descriptorSetLayouts, m_device, setIndex, shaderModules, resources);
}
